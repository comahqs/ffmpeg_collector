...
bool protocol_rtsp_push::do_stream(info_av_ptr p_info)
{
    ...
    if (nullptr == mp_info)
    {
        ...
        }
        {
            ...
            p_info_stream->po_code_ctx->time_base = av_inv_q(p_info_stream->pi_code_ctx->framerate); //应该根据帧率设置
    ...
    while (true)
    {
        ...
        if (AVMEDIA_TYPE_VIDEO == p_info_stream->pi_code_ctx->codec_type)
        {
            ...
            {
                av_packet_rescale_ts(p_info->p_packet, p_info_stream->pi_stream->time_base, p_info_stream->po_code_ctx->time_base);
            }
            ...
            while (true)
            {
                ...
                o_packet.stream_index = p_info_stream->index_stream;
                av_packet_rescale_ts(&o_packet, p_info_stream->po_code_ctx->time_base, p_info_stream->po_code_ctx->time_base);
