...
void protocol_rtsp_pull::handle_thread(std::string url, i_stream_ptr p_stream, std::shared_ptr<std::atomic_bool> p_bstop)
{
    ...
    p_info->p_code_cnt->framerate = av_guess_frame_rate(p_info->p_fmt_ctx, p_info->p_stream, nullptr);
    auto ret = avcodec_open2(p_info->p_code_cnt, p_info->p_decoder, nullptr);
    ...
    while (!(*p_bstop))
    {
        auto ret = av_read_frame(p_info->p_fmt_ctx, p_info->p_packet);
        if (0 > ret)
        {
            if ((ret == AVERROR_EOF) || avio_feof(p_info->p_fmt_ctx->pb))
            {
                // 已经读取完了
            }
            else
